...
class YouthMatchDataUpdateSimulatorObserver implements ISimulatorObserver {
    ...
    $fromTable = $this->_websoccer->getConfig('db_prefix') . '_youthmatch';
		...
		$result = $this->_db->querySelect($scolumns, $fromTable, $whereCondition, $match->id);
		$spiel = $result->fetch_array();
		$result->free();
    $spieltyp = $spiel['spieltyp'];
    if ($spieltyp == 'Ligaspiel') {
    $this->updateTeamstabelle($match);
		                  }
  ...
  private function updateTeamstabelle(SimulationMatch $match) {
		$fromTable = $this->_websoccer->getConfig('db_prefix') . '_verein';
		$whereCondition = 'id = %d';
		
		$tcolumns = 'ju_sa_tore,ju_sa_gegentore,ju_sa_spiele,ju_sa_siege,ju_sa_niederlagen,ju_sa_unentschieden,ju_sa_punkte';
		...
		// update statistic
		$homeColumns['ju_sa_spiele'] = $home['ju_sa_spiele'] + 1;;
		
		$homeColumns['ju_sa_tore'] = $home['ju_sa_tore'] + $match->homeTeam->getGoals();
		
		$homeColumns['ju_sa_gegentore'] = $home['ju_sa_gegentore'] + $match->guestTeam->getGoals();
		
		$guestColumns['ju_sa_spiele'] = $guest['ju_sa_spiele'] + 1;
		
		$guestColumns['ju_sa_tore'] = $guest['ju_sa_tore'] + $match->guestTeam->getGoals();
		
		$guestColumns['ju_sa_gegentore'] = $guest['ju_sa_gegentore'] + $match->homeTeam->getGoals();
		
		// assign points
		if ($match->homeTeam->getGoals() > $match->guestTeam->getGoals()) {
			$homeColumns['ju_sa_siege'] = $home['ju_sa_siege'] + 1;
			
			$homeColumns['ju_sa_punkte'] = $home['ju_sa_punkte'] + POINTS_WIN;
			
			$guestColumns['ju_sa_niederlagen'] = $guest['ju_sa_niederlagen'] + 1;
			
			$guestColumns['ju_sa_punkte'] = $guest['ju_sa_punkte'] + POINTS_LOSS;
		} else if ($match->homeTeam->getGoals() == $match->guestTeam->getGoals()) {
			$homeColumns['ju_sa_unentschieden'] = $home['ju_sa_unentschieden'] + 1;
			
			$homeColumns['ju_sa_punkte'] = $home['ju_sa_punkte'] + POINTS_DRAW;
			
			$guestColumns['ju_sa_unentschieden'] = $guest['ju_sa_unentschieden'] + 1;
			
			$guestColumns['ju_sa_punkte'] = $guest['ju_sa_punkte'] + POINTS_DRAW;
		} else {
			$homeColumns['ju_sa_niederlagen'] = $home['ju_sa_niederlagen'] + 1;
			
			$homeColumns['ju_sa_punkte'] = $home['ju_sa_punkte'] + POINTS_LOSS;
			
			$guestColumns['ju_sa_siege'] = $guest['ju_sa_siege'] + 1;
			
			$guestColumns['ju_sa_punkte'] = $guest['ju_sa_punkte'] + POINTS_WIN;
		}
